...
<div class="container-fluid">
		<div class="row">
			<div class="termos">
				...
				<p class="h1 text-center mt-5">Painel de Controle</p>
			</div>
		</div>
...
<div class="mx-auto row container-fluid agenda row-agenda m-0 p-0">
...
{
	$consulta = "SELECT * FROM tb_evento as e
	...
	"SELECT * FROM tb_evento AS e
		...
		JOIN tb_uf as u
			ON c.cd_uf = u.cd_uf
				WHERE cl.cd_cliente = $cd_cliente
		ORDER BY cd_evento DESC
			LIMIT $inicio, $qt");
}
...
?>
		...
		<div class="mx-auto mb-3">
			<a href="alt-evento?id=<?php echo $cd_evento ?>">
				<button type="button" value="alterar" class="btn btn-primary">ALTERAR</button>
			</a>

			<a href="excluir-evento?id=<?php echo $cd_evento ?>">
...
else
{
	...
	<div class="text-center mx-auto mt-5">
		<h2 class="h2">Nenhum evento cadastrado,</h2>
		<h2 class="h2 text-primary">
			<a href="cad-eventos" class="text-primary">Clique aqui e comece agora mesmo!</a>
		</h2>
	</div>
	<?php
}
			...
			<div class="row">
				...
				<nav aria-label="Page navigation example">
				  <ul class="pagination">
				    ...
				    <?php
				   	if($anterior != 0){ ?>
				      <a class="page-link" href="painel.php?pagina=<?php echo $anterior; ?>" aria-label="Previous">
				    ...
				    <?php
				   	for($i = 1; $i < $num + 1; $i++)
				   	{?>
						<li class="page-item"><a class="page-link" href="painel.php?pagina=<?php echo $i; ?>">
							<?php echo $i; ?>
						</a></li>
					<?php } ?>
				    <li class="page-item">
				    <?php
				   	if($posterior <= $num){ ?>
				      <a class="page-link" href="painel.php?pagina=<?php echo $posterior; ?>" aria-label="Next">
